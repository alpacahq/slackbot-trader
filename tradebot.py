        ...
        if(args[0].lower() == "market"):
            ...
            try:
                ...
                order = api.submit_order(
                    args[3], args[2], args[1], args[0], args[4])
    ...
    if(args[0] == "positions"):
        ...
        try:
            ...
            if(len(orders) == 0):
                return "No orders."
            orders = map(
                lambda x: (f'Symbol: {x.symbol}, Qty: {x.qty}, Side: {x.side}, Type: {x.type}, Amount filled: {x.filled_qty}{(f", Stop Price = {x.stop_price}","")[x.stop_price == None]}{(f", Limit Price = {x.limit_price}","")[x.limit_price == None]}'),
                orders)
